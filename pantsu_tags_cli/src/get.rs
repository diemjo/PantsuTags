...
fn link_files_to_tmp_dir(files: &Vec<ImageFile>, lib_dir: &Path, tmp_path: &PathBuf) -> AppResult<()> {
...
fn print_file_paths(files: &Vec<ImageFile>, lib_dir: &Path) -> AppResult<()> {
...
fn get_file_paths(files: &Vec<ImageFile>, lib_dir: &Path) -> AppResult<Vec<PathBuf>> {
